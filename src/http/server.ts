...
const plaidRemoveSchema = z.object({
...
const alertRuleRemoveSchema = z.object({
  id: z.string().uuid(),
}).strict();

const alertMuteCreateSchema = z.object({
...
async function route(
  ...
  if (url.pathname === '/v1/plaid/remove' && method === 'POST') {
    const input = parseSchema(plaidRemoveSchema, await readJson(request));
    return sendJson(response, 200, await service.removePlaidConnection(input.item_id));
  }
  ...
  if (url.pathname === '/v1/chat' && method === 'POST') {
    ...
    return sendJson(response, 200, await service.chat(input.messages, input.section));
  }
  ...
  if (url.pathname === '/v1/alert-rules/toggle' && method === 'POST') {
    const input = parseSchema(alertRuleToggleSchema, await readJson(request));
    return sendJson(response, 200, service.toggleAlertRule(input.id, input.enabled));
  }
  if (url.pathname === '/v1/alert-rules/remove' && method === 'POST') {
    const input = parseSchema(alertRuleRemoveSchema, await readJson(request));
    return sendJson(response, 200, service.removeAlertRule(input.id));
  }
  if (url.pathname === '/v1/alert-mutes' && method === 'GET') {
    return sendJson(response, 200, { items: service.listAlertMutes() });
  }
  if (url.pathname === '/v1/alert-mutes' && method === 'POST') {
    const input = parseSchema(alertMuteCreateSchema, await readJson(request));
    return sendJson(response, 201, service.createAlertMute(input));
  }
  if (url.pathname === '/v1/alert-mutes/remove' && method === 'POST') {
    const input = parseSchema(alertRuleRemoveSchema, await readJson(request));
    return sendJson(response, 200, service.removeAlertMute(input.id));
  }
